...
namespace CFStudio.Localization
{
    ...
    public static class DateTimeExtensions
    {
        ...
        public static string PluralizeInt(string keyword, int value)
        {
            ...
            if (value > 1)
            {
                return keyword + suffix.Replace(".", string.Empty);
            }
        ...
        public static string ToLongDuration(this TimeSpan span)
        {
            string formatted = string.Format(
                "{0}{1}{2}", 
                span.Days > 0
                    ? string.Format("{0:0} {1} ", span.Days, PluralizeInt(Resources.Day, span.Days))
                    : string.Empty, 
                span.Hours > 0
                    ? string.Format("{0:0} {1} ", span.Hours, PluralizeInt(Resources.Hour, span.Hours))
                    : string.Empty, 
                span.Minutes > 0
                    ? string.Format("{0:0} {1} ", span.Minutes, PluralizeInt(Resources.Minute, span.Minutes))
                    : string.Empty);
        ...
        public static string ToShortDuration(this TimeSpan span)
        {
            string formatted = string.Format(
                ...
                span.Minutes > 0
                    ? string.Format("{0:0}{1} ", span.Minutes, Resources.Minute.ToLower()[0])
                    : string.Empty);
            return formatted.Trim();
        }

        /// <summary>
        /// The to short month name.
        /// </summary>
        /// <param name="dateTime">
        /// The date time.
        /// </param>
